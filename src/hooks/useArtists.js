...
export function useArtists({ search = '', genre = '', limit = 100 } = {}) {
  ...
  const transformedData = artistsData?.map(artist => {
    ...
    return {
      ...
      totalConcerts,
      // Canonical Popularity, read straight from artists.popularity (weekly-
      // cache-plus-manual-sync design, 2026-09) -- the same column the
      // background scheduler and the "Sync Now" button both keep fresh.
      // This used to be a separate live call to the Python engine on every
      // page load (15-22s); normal browsing now never waits on that at all.
      // null (not yet computed) → "—", never fabricated as 0.
      popularity: artist.popularity != null ? Number(artist.popularity) : null,
