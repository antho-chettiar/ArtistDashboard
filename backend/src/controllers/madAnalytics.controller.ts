...
export const madAnalyticsController = {
  ...
  // Dashboard homepage highlights (spotlight + revisit reminders) -- a
  // single cheap SQL query on the Python side, no caching layer needed.
  getDashboardHighlights: async (_req: Request, res: Response) => {
    try {
      const result = await madAnalyticsService.getDashboardHighlights();
      return res.status(200).json({ success: true, data: result });
    } catch (error) {
      return handleAnalyticsError(res, error, 'getDashboardHighlights');
    }
  },
  ...
  getTopInsightPerArtist: async (_req: Request, res: Response) => {
    try {
      const result = await madAnalyticsService.getTopInsightPerArtist();
      return res.status(200).json({ success: true, data: result });
    } catch (error) {
      return handleAnalyticsError(res, error, 'getTopInsightPerArtist');
    }
  },
